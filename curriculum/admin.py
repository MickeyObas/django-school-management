...
class CourseCodePrefixFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        course_code_prefixs = set(
            [course.course_code_prefix for course in model_admin.model.objects.all()]
        )
        return [
            (course_code_prefix, course_code_prefix)
            for course_code_prefix in course_code_prefixs
        ]
...
class DepartmentLevelCoursePackAdmin(admin.ModelAdmin):
    list_display = ["department", "level", "course_list"]
    readonly_fields = ["course_list"]
